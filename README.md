...
```text
┌ HongKong VPS · 2026-08-29 23:00–2026-08-30 00:00
│ 本时段：1.25 GiB
│ 2026-08-29 全日累计：28.60 GiB
└ 安装后总计：46.32 GiB
```

## 工作方式

脚本以安装时的 vnStat RX/TX 累计值为基线，之后只累计新增流量，因此不会把安装前的流量算入“总消耗”。所有推送、小时区间和每日归零统一使用北京时间（Asia/Shanghai）。若首次安装不在整点，第一条记录会显示实际的部分小时，例如 `2026-08-29 09:37–10:00`；之后为完整小时。

流量单位按 GiB 计算（1 GiB = 1024³ 字节），结果保留两位小数。统计对象是所选网卡；容器虚拟网卡、内网专用网卡等不会自动合并。
